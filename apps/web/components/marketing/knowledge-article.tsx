...
export function KnowledgeArticle({
  ...
  return (
    <article className="bg-background">
      {/* HERO */}
      <header className="border-b border-iron-200 bg-iron-50/60 dark:border-dlugomat-800 dark:bg-dlugomat-950/40">
        <div className="container py-12 sm:py-16">
          <div className="mx-auto max-w-3xl">
            ...
            <p className="mt-6 text-fluid-lg leading-relaxed text-iron-700 dark:text-iron-200">
              {lead}
            </p>
          </div>
        </div>
      ...
      <div className="container py-12 sm:py-16">
        <div className="mx-auto grid max-w-5xl gap-12 lg:grid-cols-[260px_1fr]">
          ...
          <aside className="lg:sticky lg:top-24 lg:self-start">
            <p className="text-fluid-xs font-semibold uppercase tracking-wider text-iron-500">
              Spis treści
            </p>
              ...
              <ol className="flex flex-col gap-2">
                {sections.map((section, i) => (
                  <li key={section.id}>
                    <a
                      href={`#${section.id}`}
                      className="block rounded-md px-2 py-1 text-fluid-sm text-iron-600 transition-colors hover:bg-iron-100 hover:text-dlugomat-700 dark:text-iron-300 dark:hover:bg-dlugomat-850"
                    >
          ...
          <div className="prose prose-iron max-w-none dark:prose-invert">
            {sections.map((section) => (
              <section key={section.id} id={section.id} className="scroll-mt-24">
                ...
                <div className="mt-4 text-fluid-base leading-relaxed text-iron-700 dark:text-iron-200">
                  {section.content}
                </div>
              </section>
            ))}
            ...
            {legalSources.length > 0 ? (
              <section className="mt-16 rounded-xl border border-iron-200 bg-iron-50/60 p-6 dark:border-dlugomat-800 dark:bg-dlugomat-950/40">
                <h2 className="!mt-0 text-fluid-lg font-semibold text-dlugomat-900 dark:text-iron-50">
                  Źródła prawne
                </h2>
                <ul className="mt-3 flex flex-col gap-1.5 text-fluid-sm text-iron-600 dark:text-iron-300">
      ...
      {/* RELATED MODULE */}
      {relatedModule ? (
        <section className="bg-iron-50/60 py-16 dark:bg-dlugomat-950/40">
            ...
            <div className="mt-8 rounded-xl border border-iron-200 bg-card p-6 sm:p-8 dark:border-dlugomat-800">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div className="flex flex-col gap-2">
                  ...
                  <h3 className="text-fluid-xl font-semibold text-dlugomat-900 dark:text-iron-50">
                    {relatedModule.title}
                  </h3>
                </div>
      ...
      <section className="container py-12">
        <div className="mx-auto max-w-3xl rounded-xl border border-iron-200 bg-card p-5 text-fluid-sm leading-relaxed text-iron-600 dark:border-dlugomat-800 dark:text-iron-300">
          <p className="!my-0">
            <strong className="font-semibold text-dlugomat-900 dark:text-iron-50">
              Zastrzeżenie:
            </strong>{" "}
