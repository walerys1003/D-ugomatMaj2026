...
function RankIcon({ rank }: { rank: number }) {
  ...
  if (rank === 2) return <Trophy className="h-5 w-5 text-iron-400" aria-hidden />;
  if (rank === 3) return <Medal className="h-5 w-5 text-iron-500" aria-hidden />;
  return null;
}
...
export default function PoleceniaRankingPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      <nav aria-label="Widoki poleceń" className="flex gap-1 rounded-md border border-iron-200 bg-iron-50 p-1 w-fit text-sm">
        <Link
        ...
        <Card urgency="success">
          <CardContent className="flex flex-wrap items-center gap-4 p-5">
            ...
            <div className="flex-1 min-w-[180px]">
              <p className="text-xs uppercase tracking-wide text-iron-500">Twoje miejsce</p>
              <p className="font-display text-fluid-h3 text-dlugomat-950">
                #{you.rank} · {you.alias}
              </p>
            </div>
            <div className="text-right">
              <p className="text-xs uppercase tracking-wide text-iron-500">Zarobione</p>
        ...
        {RANKING.slice(0, 3).map((r) => (
          ...
          >
            ...
            <CardContent className="space-y-2 text-sm">
              <div className="flex items-center justify-between">
                <span className="text-iron-600">Polecenia</span>
                <span className="font-semibold text-dlugomat-900">{r.invites}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-iron-600">Konwersje</span>
                <span className="font-semibold text-dlugomat-900">{r.conversions}</span>
              </div>
              <div className="flex items-center justify-between border-t border-iron-100 pt-2">
                <span className="text-iron-600">Zarobione</span>
              ...
              <tbody className="divide-y divide-iron-100 bg-white">
                {RANKING.map((r) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-2 text-iron-600">{r.region}</td>
