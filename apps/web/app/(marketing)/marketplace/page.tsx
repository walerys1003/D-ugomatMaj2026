...
function StarRating({ avg, count }: { avg: number; count: number }) {
  ...
  return (
    <span className="inline-flex items-center gap-1 text-fluid-xs text-ink-600 dark:text-ink-300">
      <span aria-hidden className="text-warn-500">
        {"★".repeat(full)}
        <span className="text-ink-300 dark:text-dlugomat-700">{"★".repeat(5 - full)}</span>
      </span>
      <span>
        {avg.toFixed(1)} <span className="text-ink-400">({count})</span>
      </span>
    </span>
  );
}
...
function ListingCard({ l }: { l: MarketplaceListing }) {
  return (
    ...
    >
      ...
      />
      <div className="aspect-[16/9] w-full overflow-hidden bg-ink-50 dark:bg-dlugomat-900">
        {l.cover_image_url ? (
          ...
          <div className="flex h-full w-full items-center justify-center text-ink-300 dark:text-dlugomat-700">
      ...
      <CardContent className="flex flex-1 flex-col gap-3 p-5">
        ...
        <h3 className="font-display text-fluid-lg leading-tight text-dlugomat-900 dark:text-ink-50">
          {l.title}
        </h3>
        <p className="line-clamp-2 text-fluid-sm text-ink-600 dark:text-ink-300">{l.tagline}</p>

        <div className="mt-auto flex items-center justify-between border-t border-ink-200 pt-3 dark:border-dlugomat-800">
          <span className="text-fluid-xs text-ink-500 dark:text-ink-400">{l.vendor.name}</span>
          <StarRating avg={l.rating_avg} count={l.rating_count} />
        </div>
        <div className="flex items-center justify-between text-fluid-xs text-ink-500 dark:text-ink-400">
          <span>
            {PRICE_MODEL_LABELS[l.price_model]}
            {l.price_pln != null && l.price_pln > 0 && (
              <span className="ml-1 font-semibold text-dlugomat-900 dark:text-ink-50">
                · {l.price_pln} zł
              </span>
            )}
          </span>
    ...
    <div className="bg-gradient-to-b from-ink-50 to-white pb-20 dark:from-dlugomat-950 dark:to-dlugomat-900">
      ...
      {featured.length > 0 && (
        <section className="container py-8">
          <h2 className="mb-6 font-display text-fluid-2xl font-semibold text-dlugomat-900 dark:text-ink-50">
      ...
      <section className="container py-8">
        <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
          <h2 className="font-display text-fluid-2xl font-semibold text-dlugomat-900 dark:text-ink-50">
            {params.category
              ? LISTING_CATEGORIES.find((c) => c.key === params.category)?.label ?? "Kategoria"
              : "Wszystkie pozycje"}
            <span className="ml-3 text-fluid-base font-normal text-ink-500">
              {rest.length + featured.length}
            </span>
          </h2>
          <div className="flex items-center gap-2 text-fluid-sm">
            <span className="text-ink-500">Sortuj:</span>
            <Link
              href={`/marketplace?${new URLSearchParams({ ...params, sort: "popular" } as never).toString()}`}
              className={params.sort === "popular" || !params.sort ? "font-semibold text-dlugomat-700" : "text-ink-600 hover:text-dlugomat-700"}
            >
              Popularne
            </Link>
            ...
            <Link
              ...
              className={params.sort === "newest" ? "font-semibold text-dlugomat-700" : "text-ink-600 hover:text-dlugomat-700"}
            >
              Najnowsze
            </Link>
            <span aria-hidden className="text-ink-300">·</span>
            <Link
              ...
              className={params.sort === "rating" ? "font-semibold text-dlugomat-700" : "text-ink-600 hover:text-dlugomat-700"}
            >
              Najwyżej oceniane
            </Link>
          </div>
        ...
        {rest.length === 0 && featured.length === 0 ? (
          <Card elevation="subtle" className="py-16 text-center">
            <CardContent>
              <p className="font-display text-fluid-xl text-ink-700 dark:text-ink-200">
                Brak pozycji w tej kategorii
              </p>
              <p className="mt-2 text-fluid-sm text-ink-500">Pracujemy nad nowymi integracjami. Wróć wkrótce.</p>
      ...
      <section className="container mt-16">
        <Card elevation="pop" className="overflow-hidden border-dlugomat-700/30 bg-gradient-to-br from-dlugomat-900 to-dlugomat-700 text-ink-50">
          <CardContent className="grid gap-8 p-10 lg:grid-cols-[2fr_1fr] lg:items-center">
            <div>
              ...
              <p className="mt-4 max-w-xl text-fluid-base leading-relaxed text-ink-100/90">
            ...
            <div className="flex flex-col gap-3">
              ...
              <Button asChild size="lg" variant="ghost" className="w-full border border-ink-50/30 text-ink-50 hover:bg-ink-50/10">
