        ...
        <div className="mt-10 grid grid-cols-2 gap-4 sm:grid-cols-4">
          <Card elevation="flat">
            <CardContent className="p-5">
              <p className="font-display text-fluid-3xl font-bold text-dlugomat-900 dark:text-ink-50">
                {partners.length}
              </p>
          ...
          <Card elevation="flat">
            <CardContent className="p-5">
              ...
              <p className="text-fluid-xs uppercase tracking-wide text-ink-500">Województw</p>
            </CardContent>
          </Card>
          <Card elevation="flat">
            <CardContent className="p-5">
          ...
          <div className="space-y-12">
            {Object.entries(byVoivodeship)
              ...
              .map(([voivodeship, items]) => (
                <div key={voivodeship}>
                  <h2 className="mb-4 font-display text-fluid-xl font-semibold text-dlugomat-900 dark:text-ink-50">
                    {voivodeship}
                      ...
                      <Card key={p.id} elevation="subtle" className="group transition-shadow hover:shadow-pop">
                        ...
                        >
                          <div className="flex items-start gap-4">
                            <div className="h-12 w-12 shrink-0 overflow-hidden rounded-lg bg-ink-100 dark:bg-dlugomat-900">
                              {p.logo_url ? (
                                // eslint-disable-next-line @next/next/no-img-element
                                <img src={p.logo_url} alt="" className="h-full w-full object-cover" />
                              ) : (
                                <span className="flex h-full w-full items-center justify-center font-display text-fluid-xl text-ink-400">
                                  {p.name.charAt(0)}
                                </span>
                              )}
                            </div>
                            <div className="min-w-0 flex-1">
                              <h3 className="truncate font-display text-fluid-base font-semibold text-dlugomat-900 dark:text-ink-50">
                                {p.name}
                              </h3>
                              <p className="mt-0.5 text-fluid-xs text-ink-500">
                                {TYPE_LABELS[p.type]} · {p.city}
                              </p>
                            </div>
                          </div>
                          <p className="mt-3 line-clamp-2 text-fluid-sm text-ink-600 dark:text-ink-300">
                            {p.tagline}
                          </p>
                          ...
                          <div className="mt-4 flex items-center justify-between border-t border-ink-200 pt-3 text-fluid-xs text-ink-500 dark:border-dlugomat-800">
                            <span>
                              <span className="text-warn-500">★</span> {p.rating_avg.toFixed(1)}{" "}
                              <span className="text-ink-400">({p.rating_count})</span>
                            </span>
      ...
      <section className="container mt-16">
        <Card elevation="pop" className="border-dlugomat-700/30 bg-dlugomat-50 dark:bg-dlugomat-900">
          <CardContent className="grid gap-6 p-10 lg:grid-cols-[2fr_1fr] lg:items-center">
            <div>
              <h2 className="font-display text-fluid-2xl font-bold text-dlugomat-900 dark:text-ink-50">
                Jesteś kancelarią lub radcą prawnym?
              </h2>
              <p className="mt-3 text-fluid-base leading-relaxed text-ink-700 dark:text-ink-200">
