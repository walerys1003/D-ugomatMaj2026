    ...
    <main className="bg-iron-50 dark:bg-iron-950 pb-20">
      <section className="bg-white dark:bg-iron-900 border-b border-iron-200 dark:border-iron-800">
        <div className="container mx-auto px-4 py-12 max-w-4xl">
          <Link
            href="/integracje"
            className="text-xs text-iron-500 hover:text-iron-700"
          >
            ← Wszystkie integracje
          </Link>
          <div className="flex items-start gap-4 mt-3">
            <div className="w-14 h-14 rounded-lg bg-iron-100 dark:bg-iron-800 flex items-center justify-center font-display text-2xl font-semibold text-iron-700 dark:text-iron-300 flex-shrink-0">
              {d.logo_letter}
            </div>
            <div className="flex-1">
              <p className="text-xs uppercase tracking-wider text-iron-500 mb-1">
                {d.category}
              </p>
              <h1 className="font-display text-3xl md:text-4xl font-semibold text-iron-900 dark:text-iron-50">
                {d.name}
              </h1>
              <p className="text-lg text-iron-600 dark:text-iron-300 mt-2">{d.tagline}</p>
              <div className="flex gap-2 mt-3">
                <span className="text-xs px-2 py-1 rounded-full bg-accent-50 text-accent-700 border border-accent-200">
                  {STATUS_LABEL[d.status]}
                </span>
                <span className="text-xs px-2 py-1 rounded-full bg-iron-100 dark:bg-iron-800 text-iron-700 dark:text-iron-300">
      ...
      <section className="container mx-auto px-4 py-10 max-w-4xl grid lg:grid-cols-[1fr_280px] gap-6">
        <div className="space-y-6">
          <Card elevation="subtle">
            ...
            <CardContent>
              <p className="text-iron-700 dark:text-iron-300">{d.description}</p>
            </CardContent>
          </Card>
          ...
          <Card elevation="subtle">
            ...
            <CardContent>
              <ul className="space-y-2 text-sm text-iron-700 dark:text-iron-300">
                {d.features.map((f, i) => (
          ...
          <Card elevation="subtle">
            ...
            <CardContent>
              <ol className="space-y-3 text-sm text-iron-700 dark:text-iron-300">
        ...
        <aside className="space-y-4">
          ...
          <Card elevation="subtle">
            <CardContent className="pt-5 text-xs text-iron-600 dark:text-iron-400 space-y-2">
              <div>
                <strong className="text-iron-700 dark:text-iron-300">Plan:</strong>{" "}
                {PLAN_LABEL[d.required_plan]} i wyższe
              </div>
              <div>
                <strong className="text-iron-700 dark:text-iron-300">Status:</strong>{" "}
                {STATUS_LABEL[d.status]}
              </div>
              <div>
                <strong className="text-iron-700 dark:text-iron-300">Wsparcie:</strong>{" "}
