  ...
  if (!data) {
    return (
      <main className="container mx-auto px-4 py-12 max-w-6xl">
        <p className="text-ink-600">Brak danych NPS.</p>
      </main>
    );
  }
              ...
              <tbody>
                {data.by_segment.map((s) => (
                  ...
                  >
                    ...
                    <td className="py-2 pr-3 text-right text-ink-500">{s.responses}</td>
                    <td
          ...
          <CardContent>
            <ul className="space-y-2">
              {data.top_themes.map((t, i) => (
                ...
                >
                  <span className="text-ink-900 dark:text-ink-50">{t.theme}</span>
        ...
        <CardContent>
          <ul className="space-y-3">
            {data.recent_verbatims.map((v) => (
              <li
                key={v.id}
                className="rounded-lg border border-ink-200 dark:border-ink-800 p-3"
              >
