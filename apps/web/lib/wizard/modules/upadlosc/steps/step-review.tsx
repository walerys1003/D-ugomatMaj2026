...
export function StepReview({
  ...
  return (
    <form
      ...
      noValidate
    >
      <div className="rounded-lg border border-temporal-amber-200 bg-temporal-amber-50/40 p-4 text-sm text-iron-800">
      ...
      <section className="rounded-lg border border-iron-200 bg-white p-4 text-sm text-iron-800 shadow-sm">
        <h3 className="mb-2 text-fluid-sm font-semibold text-iron-900">
          Wnioskodawca
        </h3>
        <dl className="grid gap-1">
          <div className="flex gap-2">
            <dt className="text-iron-600">Imię i nazwisko:</dt>
            <dd>{a.dluznik_nazwa ?? "—"}</dd>
          </div>
          <div className="flex gap-2">
            <dt className="text-iron-600">Adres:</dt>
            <dd>{a.dluznik_adres ?? "—"}</dd>
          </div>
          <div className="flex gap-2">
            <dt className="text-iron-600">PESEL:</dt>
            <dd>
              {a.dluznik_pesel
          ...
          {a.dluznik_nip && (
            <div className="flex gap-2">
              <dt className="text-iron-600">NIP:</dt>
              <dd>{a.dluznik_nip}</dd>
            </div>
          )}
        </dl>
      ...
      <section className="rounded-lg border border-iron-200 bg-white p-4 text-sm text-iron-800 shadow-sm">
        ...
        <p>{a.sad_nazwa ?? "—"}</p>
        {a.sad_adres && <p className="text-iron-600">{a.sad_adres}</p>}
      </section>
      ...
      <section className="rounded-lg border border-iron-200 bg-white p-4 text-sm text-iron-800 shadow-sm">
        ...
        <dl className="grid gap-1">
          <div className="flex gap-2">
            <dt className="text-iron-600">Status:</dt>
            <dd>{labelFromList(STATUS_ZAWODOWY, a.status_zawodowy)}</dd>
          </div>
          <div className="flex gap-2">
            <dt className="text-iron-600">Dochód miesięczny:</dt>
            <dd>{formatPLN(a.dochod_miesieczny)}</dd>
          </div>
          <div className="flex gap-2">
            <dt className="text-iron-600">Osób na utrzymaniu:</dt>
      ...
      <section className="rounded-lg border border-iron-200 bg-white p-4 text-sm text-iron-800 shadow-sm">
        ...
        <p className="mt-2 border-t border-iron-100 pt-2 font-medium">
      ...
      <section className="rounded-lg border border-iron-200 bg-white p-4 text-sm text-iron-800 shadow-sm">
        ...
        {a.uzasadnienie && (
          <p className="mt-2 whitespace-pre-line border-t border-iron-100 pt-2 text-iron-700">
            {a.uzasadnienie}
          </p>
        )}
      </section>
